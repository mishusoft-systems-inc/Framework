...
namespace Mishusoft\Storage\FileSystem\Yaml;

use Mishusoft\Storage\FileSystem\Yaml;
use Mishusoft\Storage\FileSystem\Yaml\Exception\ParseException;
use Mishusoft\Storage\FileSystem\Yaml\Tag\TaggedValue;
...
class Parser
{
    ...
    private function doParse(string $value, int $flags)
    {
        ...
        do {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                if (!empty($context) && 'mapping' === $context) {
                    throw new ParseException(
                ...
                if (isset($values['value'][1]) && '?' === $values['value'][0] && ' ' === $values['value'][1]) {
                    throw new ParseException(
                        'Complex mappings are not supported.',
                        $this->getRealCurrentLineNb() + 1,
                        $this->currentLine
                    );
                }
            ...
            } elseif (self::preg_match('#^(?P<key>(?:![^\s]++\s++)?(?:'.Inline::REGEX_QUOTED_STRING.'|(?:!?!php/const:)?[^ \'"\[\{!].*?)) *\:(( |\t)++(?P<value>.+))?$#u', rtrim($this->currentLine), $values)
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ('<<' === $key) {
                            ...
                            throw new ParseException(
                                sprintf('Duplicate key "%s" detected.', $key),
                                $realCurrentLineNbKey + 1,
                                $this->currentLine
                            );
                        }
                    }
    ...
    private function parseBlockScalar(string $style, string $chomping = '', int $indentation = 0): string
    {
        ...
        if ('>' === $style) {
            ...
            foreach ($blockLines as $i => $iValue) {
                if ('' === $iValue) {
                    $text .= "\n";
                    $previousLineIndented = false;
                    $previousLineBlank = true;
    ...
    private function getLineTag(string $value, int $flags, bool $nextLineCheck = true): ?string
    {
        ...
        if ($tag && '!' === $tag[0]) {
            throw new ParseException(
                sprintf('The built-in tag "!%s" is not implemented.', $tag),
                $this->getRealCurrentLineNb() + 1,
                $value,
                $this->filename
            );
        ...
        throw new ParseException(
            sprintf('Tags support is not enabled. You must use the flag "Yaml::PARSE_CUSTOM_TAGS" to use "%s".', $matches['tag']),
            $this->getRealCurrentLineNb() + 1,
            $value,
            $this->filename
        );
    ...
    private function lexInlineQuotedString(int &$cursor = 0): string
    {
        ...
        do {
            ...
            for ($cursorMax = \strlen($this->currentLine); $cursorMax > $cursor; ++$cursor) {
